...
db.once("open", async () => {
  try {
    // clean cuisine
    await cleanDB("Cuisine", "cuisines").then((err) => {
      err
        ? console.log("Error: ", err)
        : console.log(`---------------------\nCuisine data sanitized`);
    });
    // clean restaurant
    await cleanDB("Restaurant", "restaurants").then((err) => {
      err
        ? console.log("Error: ", err)
        : console.log("Restaurant data sanitized");
    });
    // clean user
    await cleanDB("User", "users").then((err) => {
      err ? console.log("Error: ", err) : console.log(`User data sanitized \n`);
    });
